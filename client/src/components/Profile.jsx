...
export const Profile = () => {
  ...
  const fetchprofile = () => {
    axios
      .get(`http://localhost:8080/user/${userid}/posts`)
      .then((res) => {
  ...
  const handlesubmit = () => {
    if (title.length > 0 && url.length > 0 && description.length > 0) {
      ...
      axios
        .post(`http://localhost:8080/user/${userid}/post`, payload)
        .then((res) => {
  ...
  return (
    <div>
      <div className={styles.addpost}>
        <h2 id={styles.addposthere} onClick={changevisibility}>
          Add Post here...
        </h2>
